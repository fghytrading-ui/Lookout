...
export default function DashboardPage({ market = 'stocks', title = null }) {
  ...
  return (
    <div className="pb-14">
      ...
      <header className="sticky top-[var(--nav-h,52px)] z-40 bg-[#080808]/95 backdrop-blur border-b border-[#151515]">
      ...
      {/* Macro blackout — a top-tier release is imminent so new entries are
          barred. Shown prominently: an empty ENTER NOW with no reason reads
          as a broken scanner. */}
      {scanStats.macroBlackout && (
        <div className="mx-4 mt-3 p-3 rounded border border-amber-500/50 bg-amber-500/10 text-amber-200 font-mono warn-pulse">
          ...
          <div className="text-[10px] opacity-70 mt-1">
            Setups below stay visible so you can prepare, but none are recommended for entry right now.
          </div>
        </div>
      )}
        ...
        {loading ? (
          ...
          <>
            ...
            {(() => {
              ...
              // just a softer version of showing trades nobody should take.
              return (
                <>
